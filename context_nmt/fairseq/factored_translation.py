...
from context_nmt.fairseq.factored_language_pair_dataset import (
...
class FactoredTranslationTask(FairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--add-source-factor', action='store_false',
                            help='add segment id to source')
        # fmt: on
    ...
    def _inference_with_bleu(self, generator, sample, model):
        ...
        def decode(toks, escape_unk=False):
            s = self.tgt_dict.string(
                toks.int().cpu(), self.args.eval_bleu_remove_bpe, escape_unk=escape_unk,
            )
            if self.tokenizer:
        ...
        tokenize = (
            sacrebleu.DEFAULT_TOKENIZER if not self.args.eval_tokenized_bleu else "none"
        )
