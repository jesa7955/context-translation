...
@Model.register("factored_composed_seq2seq")
class FactoredComposedSeq2Seq(Model):
    """
    ...
    def __init__(
        ...
        decoder: SeqDecoder,
        source_factor_method: str = "concat",
        source_factor_embedder: TextFieldEmbedder = None,
        ...
        self._source_factor_embedder = source_factor_embedder
        self._source_factor_method = source_factor_method
    ...
    def forward(
        ...
        source_factors: Dict[str, torch.LongTensor] = None,
        ...
        """
        state = self._encode(source_tokens, source_factors)
    ...
    def _encode(
        ...
        source_factors: Dict[str, torch.Tensor] = None,
    ) -> Dict[str, torch.Tensor]:
        """
        ...
        if source_factors:
            embedded_factors = self._source_factor_embedder(source_factors)
            if self._source_factor_method == "concat":
                embedded_input = torch.cat((embedded_input, embedded_factors), -1)
            elif self._source_factor_method == "add":
                embedded_input = embedded_input + embedded_factors
