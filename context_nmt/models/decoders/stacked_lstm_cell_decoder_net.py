...
@DecoderNet.register("stacked_lstm_cell")
class StackedLstmCellDecoderNet(DecoderNet):
    """
    ...
    def __init__(
        ...
        self._decoder_cell = torch.nn.ModuleList(
            [
                LSTMCell(
                    input_size=decoder_input_dim if layer == 0 else hidden_dim,
    ...
    def forward(
        ...
        layer_intput = decoder_input
        layer_hidden = decoder_hidden
        layer_context = decoder_context
        for layer in self._decoder_cell:
            layer_hidden, layer_context = layer(
                layer_input, (layer_hidden, layer_context)
            )
